...
def retrieve_img(url):
    ...
    try:
        ...
        img = soup.find(class_='entry-image').find("img")['src']
        flag = 'Success'
...
def draw_progress_bar(percent, status='', bar_len = 40):
    sys.stdout.write("\r")
    sys.stdout.write("[{:<{}}] {:>3.0f}%       {:20}".format("=" * int(bar_len * percent), bar_len, percent * 100, status))
    sys.stdout.flush()